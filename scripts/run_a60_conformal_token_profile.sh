...
for required in "$A57_RUN/PROBE_COMPLETE" \
  ...
  "$SOURCE_DIR/scripts/run_a60_conformal_token_profile_worker.sh" "$ROOT/scripts/gpu_guard.py"; do
  [[ -e "$required" ]] || { echo "Missing A60 input: $required" >&2; exit 2; }
done

for scene in ramen figurines teatime waldo_kitchen; do
  if [[ -f "$RUN_ROOT/$scene/eval_conformal_token_profile_completion/metrics.json" ]]; then continue; fi
  "$PYTHON_BIN" "$ROOT/scripts/gpu_guard.py" --gpu "$GPU" --hold-mb 384 \
    ...
    env ROOT="$ROOT" SOURCE_DIR="$SOURCE_DIR" RUN_ROOT="$RUN_ROOT" LOG_DIR="$LOG_DIR" \
      ...
      bash "$SOURCE_DIR/scripts/run_a60_conformal_token_profile_worker.sh" \
...
for scene in ("ramen", "figurines", "teatime", "waldo_kitchen"):
    ...
    candidate = row(os.path.join(root, scene, "eval_conformal_token_profile_completion", "metrics.json"))
    scenes[scene] = {"reference": prior["reference"], "a55": prior["a55"], "a57": prior["a57"], "a60": candidate}
    for method in ("reference", "a55", "a57"):
        scenes[scene]["delta_from_" + method] = {k: candidate[k] - scenes[scene][method][k] for k in names}
summary = {
    "experiment": "A60_conformal_relative_token_profile_group_completion",
    "fixed_seed": int(seed),
    ...
    "profile_contract": {
        "four_equal_resident_tokens": True,
        "query_response_profile": "per-Gaussian L0-L3 scores normalized by local maximum",
        "adaptive_radius": "seed 0.90 quantile plus 0.03 margin",
        "minimum_common_slots": 2,
        "resident_tokens_unchanged": True,
    },
    "scenes": scenes,
}
for method in ("reference", "a55", "a57", "a60"):
    for metric in names:
        summary[f"mean_{method}_{metric}"] = sum(x[method][metric] for x in scenes.values()) / 4
...
PY
echo "A60 conformal token-profile completion complete: $RUN_ROOT"
