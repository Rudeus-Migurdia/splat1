...
RUN_ROOT=${RUN_ROOT:?RUN_ROOT must name an isolated A54 run directory}
LOG_DIR=${LOG_DIR:?LOG_DIR must name an isolated A54 log directory}
...
A53_RUN=${A53_RUN:-$ROOT/runs/a53_global_sparse_group_retrieval_20260721_220010}
...
for required in \
  "$A52_RUN/PROBE_COMPLETE" "$A53_RUN/PROBE_COMPLETE" \
  ...
  [[ -e "$required" ]] || { echo "Missing A54 input: $required" >&2; exit 2; }
done
...
print("A54_CONFORMAL_GROUP_INPUT_CONTRACT_OK")
PY
...
evaluate() {
  ...
  CUDA_VISIBLE_DEVICES=$GPU "$PYTHON_BIN" -u \
    ...
    --global_group_anchor_quantile 0.20 --global_group_outside_quantile 0.75 \
...
evaluate anchor equal_query_global_anchor_entmax15
evaluate anchor_null equal_query_global_anchor_null_entmax15
evaluate anchor_null_geodesic equal_query_global_anchor_null_entmax15_geodesic

"$PYTHON_BIN" - "$RUN_ROOT" "$A33_RUN" "$A51_RUN" "$A52_RUN" "$A53_RUN" <<'PY'
import json, os, sys
root, a33, a51, a52, a53 = sys.argv[1:]
...
paths = {
    ...
    "a53_global_entmax15_geodesic": os.path.join(a53, "ramen", "eval_global_entmax15_geodesic", "metrics.json"),
    "a54_anchor": os.path.join(root, "ramen", "eval_anchor", "metrics.json"),
    "a54_anchor_null": os.path.join(root, "ramen", "eval_anchor_null", "metrics.json"),
    "a54_anchor_null_geodesic": os.path.join(root, "ramen", "eval_anchor_null_geodesic", "metrics.json"),
}
metrics = {name: row(path) for name, path in paths.items()}
reference = metrics["a33"]
variants = ("a54_anchor", "a54_anchor_null", "a54_anchor_null_geodesic")
...
summary = {
    ...
    "checks": {
        ...
        "best_bowl_beats_a51": metrics[winner]["per_category"]["bowl"] >= metrics["a51_composite_refine"]["per_category"]["bowl"],
    },
}
...
echo "A54 conformal Group anchor complete: $RUN_ROOT"
