#!/usr/bin/env bash
set -euo pipefail

ROOT=${ROOT:-/mnt/zju105100171/home/anlanfan/Dr-Splat}
PYTHON_BIN=${PYTHON_BIN:-$ROOT/.venv-236/bin/python}
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/a6_responsibility_adaptive_readout_20260715}
LOG_DIR=${LOG_DIR:-$ROOT/logs/a6_responsibility_adaptive_readout_20260715}

cd "$ROOT"
source scripts/drsplat_env.sh
SITE=$ROOT/.venv/lib/python3.9/site-packages
export PATH="$(dirname "$PYTHON_BIN"):$PATH"
export PYTHONPATH="$ROOT:$SITE:$SITE/setuptools/_vendor:${PYTHONPATH:-}"
export OPENCLIP_PRETRAINED=${OPENCLIP_PRETRAINED:-$ROOT/ckpts/open_clip_vit_b16_laion2b_s34b_b88k.bin}
export HF_HUB_OFFLINE=1 HF_HUB_DISABLE_TELEMETRY=1 WANDB_MODE=offline
export OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2
mkdir -p "$RUN_ROOT" "$LOG_DIR"

base_path() {
  local scene=$1
  if [[ "$scene" == "waldo_kitchen" ]]; then
    printf '%s\n' "$ROOT/runs/multiscale_split_consistency/fused_w1p5_t005.pt"
  else
    printf '%s\n' "$ROOT/runs/multiscale_split_consistency/$scene/fused_w1p5_t005.pt"
  fi
}

candidate_path() {
  local scene=$1
  if [[ "$scene" == "waldo_kitchen" ]]; then
    printf '%s\n' "$ROOT/runs/a6_semantic_residual_waldo_20260715/m2b_split_ccl_opacity/consensus.pt"
  else
    printf '%s\n' "$ROOT/runs/a6_responsibility_multiscene_20260715/$scene/candidate/consensus.pt"
  fi
}

run_scene() {
  local scene=$1
  local base
  local candidate
  base=$(base_path "$scene")
  candidate=$(candidate_path "$scene")
  for mode in base_gate inverse_base_gate; do
    local output=$RUN_ROOT/$mode/$scene
    [[ -f "$output/metrics.json" ]] && continue
    mkdir -p "$output"
    "$PYTHON_BIN" -u eval_lerf_ovs_gaussian_codebook_miou.py \
      -s "$ROOT/drsplat_data/lerf_ovs/$scene" \
      -m "$ROOT/runs/3dgs/$scene" \
      --geometry_checkpoint "$ROOT/runs/3dgs/$scene/chkpnt30000.pth" \
      --consensus_path "$candidate" \
      --ignore_consensus_semantic_opacity \
      --consensus_blend_base "$base" \
      --consensus_candidate_weight 0.5 \
      --consensus_candidate_weight_mode "$mode" \
      --label_dir "$ROOT/drsplat_data/lerf_ovs/label/$scene" \
      --evaluation_protocol drsplat_3d_selection \
      --occupancy_threshold 0.7 \
      --output "$output" \
      > "$LOG_DIR/${mode}_${scene}.log" 2>&1
  done
}

if [[ "${1:-}" == "--worker" ]]; then
  run_scene "$2"
  exit 0
fi

scenes=(figurines ramen teatime waldo_kitchen)
pids=()
for gpu in 0 1 2 3; do
  scene=${scenes[$gpu]}
  "$PYTHON_BIN" scripts/gpu_guard.py \
    --gpu "$gpu" --hold-mb 384 --max-used-mb 256 --max-utilization 5 \
    --wait-timeout 0 -- \
    bash "$ROOT/scripts/run_a6_responsibility_adaptive_readout.sh" \
      --worker "$scene" \
    > "$LOG_DIR/worker_${scene}.log" 2>&1 &
  pids+=("$!")
done

status=0
for pid in "${pids[@]}"; do
  wait "$pid" || status=$?
done
[[ "$status" -eq 0 ]] || exit "$status"

for mode in base_gate inverse_base_gate; do
  "$PYTHON_BIN" scripts/summarize_lerf_ovs_paper.py \
    "$RUN_ROOT/$mode/figurines/metrics.json" \
    "$RUN_ROOT/$mode/ramen/metrics.json" \
    "$RUN_ROOT/$mode/teatime/metrics.json" \
    "$RUN_ROOT/$mode/waldo_kitchen/metrics.json" \
    --output "$RUN_ROOT/${mode}_four_scene_metrics.json" \
    > "$RUN_ROOT/${mode}_four_scene_table.md"
done

date +%FT%T > "$RUN_ROOT/COMPLETE"
echo "A6 adaptive responsibility readout complete: $RUN_ROOT"
