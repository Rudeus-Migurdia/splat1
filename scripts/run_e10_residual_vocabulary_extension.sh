...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/e10_residual_vocabulary_extension_20260716}
LOG_DIR=${LOG_DIR:-$ROOT/logs/e10_residual_vocabulary_extension_20260716}
...
run_scene() {
  ...
  local source_candidate=$SOURCE_ROOT/$scene/pruned_candidate_ids
  local source_base=$SOURCE_ROOT/$scene/source_artifacts/base_ids
  mkdir -p "$output"

  "$PYTHON_BIN" -u build_residual_vocabulary_extension.py \
    --consensus "$(candidate_path "$scene")" \
    --artifact_dir "$source_candidate" \
    --initial_codebook "$source_candidate/codebook_shared.npy" \
    --num_extension_codes 4096 \
    --train_samples 262144 \
    --iterations 25 \
    --assignment_chunk 4096 \
    --min_cosine_gain 0.0001 \
    --faiss_gpu \
    --seed 20260716 \
    --output_dir "$output/candidate_ids" \
    > "$LOG_DIR/${scene}_extension.log" 2>&1

  "$PYTHON_BIN" remount_shared_codebook.py \
    --artifact_dir "$source_base" \
    --codebook_path "$output/candidate_ids/codebook_shared.npy" \
...
for index in "${!scenes[@]}"; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py \
    ...
    --wait-timeout 0 -- \
    bash "$ROOT/scripts/run_e10_residual_vocabulary_extension.sh" --worker "$scene" \
...
for scene in scenes:
    rows = {}
    for name, root in (("e8_3", baseline_root), ("e10", run_root)):
    ...
    rows["delta"] = {key: rows["e10"][key] - rows["e8_3"][key] for key in rows["e10"]}
    summary["scenes"][scene] = rows
for name in ("e8_3", "e10", "delta"):
    summary[name + "_mean"] = {
...
echo "E10 residual vocabulary extension probe complete: $RUN_ROOT"
