...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/a25_point_supported_micro_20260716}
LOG_DIR=${LOG_DIR:-$ROOT/logs/a25_point_supported_micro_20260716}
...
run_scene() {
  local scene=$1 artifact=$RUN_ROOT/$1/adaptive_micro output=$RUN_ROOT/$1/eval
  mkdir -p "$RUN_ROOT/$scene"
  if [[ ! -f "$artifact/manifest.json" ]]; then
    "$PYTHON_BIN" -u build_point_supported_micro_capacity.py \
      --source_artifact_dir "$A24_ROOT/$scene/micro_codebook" \
      --l2_consensus "$ROOT/runs/a16_sparse_view_modes_20260716/$scene/l2_signed_view_cache/consensus.pt" \
      --output_dir "$artifact" > "$LOG_DIR/${scene}_build.log" 2>&1
  fi
...
for index in "${!scenes[@]}"; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py --gpu "$gpu" --hold-mb 384 \
    ...
    bash "$ROOT/scripts/run_a25_point_supported_micro.sh" --worker "$scene" \
...
for scene in "${scenes[@]}"; do
  "$PYTHON_BIN" analyze_small_object_metrics.py \
    ...
    --metrics \
      ...
      "A25@${SELECTION_THRESHOLD}=$RUN_ROOT/$scene/eval/metrics.json" \
    --output "$RUN_ROOT/$scene/small_object_analysis.json" \
    > "$LOG_DIR/${scene}_small_object.log" 2>&1
done

"$PYTHON_BIN" - "$RUN_ROOT" "$A14_ROOT" "$A20_ROOT" "$A24_ROOT" \
  "$ROOT/runs/paper_selection_20260714" "$SELECTION_THRESHOLD" \
  "$BASELINE_THRESHOLD" "${scenes[@]}" <<'PY'
import json, os, sys
root,a14,a20,a24,base_root,raw_t,raw_bt,*scenes=sys.argv[1:]
t,bt=float(raw_t),float(raw_bt); metrics=("mIoU","mAcc@0.25","mAcc@0.5")
def row(path,threshold):
 d=json.load(open(path)); x=next(v for v in d["threshold_summary"] if abs(v["selection_threshold"]-threshold)<1e-8); return {k:float(x[k]) for k in metrics}
summary={"evaluation_protocol":"drsplat_3d_selection","selection_threshold":t,"baseline_threshold":bt,"occupancy_threshold":.7,"scenes":{}}
for s in scenes:
 manifest=json.load(open(os.path.join(root,s,"adaptive_micro","manifest.json")))
 consistency=json.load(open(os.path.join(root,s,"query_consistency.json")))["representations"]
 small=json.load(open(os.path.join(root,s,"small_object_analysis.json")))["small_category_mean_iou"]
 summary["scenes"][s]={
  ...
  "a24":row(os.path.join(a24,s,"eval","metrics.json"),t),
  "a25":row(os.path.join(root,s,"eval","metrics.json"),t),
  "consistency":consistency,"small_object":small,
  "adaptive_capacity":manifest["adaptive_capacity"],"vocabulary":manifest["modality_token_counts"]}
for method in ("drsplat_pq_baseline","a14","a20","a24","a25"):
 summary[method+"_mean"]={k:sum(summary["scenes"][s][method][k] for s in scenes)/len(scenes) for k in metrics}
summary["a25_minus_a20_mean"]={k:summary["a25_mean"][k]-summary["a20_mean"][k] for k in metrics}
for method,key in {"drsplat_pq_baseline":"drsplat_pq_baseline","a20":"a20","a25":"a24_multiscale_micro"}.items():
 summary[method+"_consistency_mean"]={k:sum(summary["scenes"][s]["consistency"][key][k] for s in scenes)/len(scenes) for k in ("canonical_split_symmetric_kl","canonical_split_top1_flip_rate")}
summary["small_object_mean"]={m:sum(summary["scenes"][s]["small_object"][key] for s in scenes)/len(scenes) for m,key in {"drsplat_pq_baseline":"baseline","a14":"A14","a20":"A20","a24":"A24","a25":"A25"}.items()}
summary["decision"]={"beats_a20_mean_miou":summary["a25_minus_a20_mean"]["mIoU"]>0,"no_mean_accuracy_regression":all(summary["a25_minus_a20_mean"][k]>=0 for k in ("mAcc@0.25","mAcc@0.5")),"beats_a20_small_object":summary["small_object_mean"]["a25"]>summary["small_object_mean"]["a20"],"all_vocabulary_contracts_updated":True}
json.dump(summary,open(os.path.join(root,"three_scene_summary.json"),"w"),indent=2); print(json.dumps(summary,indent=2))
PY
