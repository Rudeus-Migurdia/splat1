...
SWEEP_DONE=$METHOD_OUT/eval/lerf_ovs_teacher_codebook_k256_coarse_fine_sweep.done
...
if [[ -f "$SWEEP_DONE" ]]; then
  echo "[$(date +%FT%T)] coarse/fine sweep already done: $SWEEP_DONE"
  exit 0
fi

while [[ ! -f "$BASELINE_METRICS" || ! -f "$METHOD_METRICS" ]]; do
  echo "[$(date +%FT%T)] waiting waldo metrics: baseline=$([[ -f "$BASELINE_METRICS" ]] && echo yes || echo no) method=$([[ -f "$METHOD_METRICS" ]] && echo yes || echo no)"
  sleep "$POLL_INTERVAL"
done

echo "[$(date +%FT%T)] waldo metrics ready; looking for idle GPU in: $GPU_CANDIDATES"
while true; do
  for gpu in $GPU_CANDIDATES; do
    if "$PYTHON_BIN" scripts/gpu_guard.py \
      ...
      bash -lc "set -euo pipefail; cd '$ROOT'; ROOT='$ROOT' VENV_PATH='$VENV_PATH' PYTHON_BIN='$PYTHON_BIN' SCENE='$SCENE' GPU_ID='$gpu' BLEND_MODES='fixed query_adaptive' AGGREGATIONS='weighted' bash scripts/run_coarse_fine_sweep.sh" \
      > "$LOG_DIR/07_waldo_coarse_fine_gpu${gpu}.log" 2>&1; then
      "$PYTHON_BIN" scripts/write_lerf_ovs_experiment_report.py \
        ...
        > "$LOG_DIR/07_waldo_report.log" 2>&1
      touch "$SWEEP_DONE"
      echo "[$(date +%FT%T)] coarse/fine sweep finished on gpu $gpu"
      exit 0
    fi
    echo "[$(date +%FT%T)] gpu $gpu not available or sweep failed; see $LOG_DIR/07_waldo_coarse_fine_gpu${gpu}.log"
  done
  echo "[$(date +%FT%T)] no idle GPU for coarse/fine sweep; retrying in $POLL_INTERVAL seconds"
  sleep "$POLL_INTERVAL"
done
