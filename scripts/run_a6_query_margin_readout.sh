...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/a6_query_margin_readout_20260715}
LOG_DIR=${LOG_DIR:-$ROOT/logs/a6_query_margin_readout_20260715}
...
candidate_path() {
  if [[ "$1" == "waldo_kitchen" ]]; then
    ...
    printf '%s\n' "$ROOT/runs/a6_responsibility_multiscene_20260715/$1/candidate/consensus.pt"
  fi
}
...
run_scene() {
  ...
  for mode in margin_switch margin_positive; do
    ...
    "$PYTHON_BIN" -u eval_lerf_ovs_gaussian_codebook_miou.py \
      ...
      --consensus_candidate_weight_mode constant \
      --consensus_query_route "$mode" \
...
for gpu in 0 1 2 3; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py \
    ...
    --wait-timeout 0 -- \
    bash "$ROOT/scripts/run_a6_query_margin_readout.sh" --worker "$scene" \
...
echo "A6 query-margin readout complete: $RUN_ROOT"
