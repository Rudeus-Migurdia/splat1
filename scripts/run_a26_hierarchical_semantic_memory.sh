...
# A26: SAM L0--L3 groups, split-consistent Old-vs-SAM source selection,
# independent level codebooks, and query-aware hierarchical memory readout.
...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/a26_hierarchical_semantic_memory_20260717}
LOG_DIR=${LOG_DIR:-$ROOT/logs/a26_hierarchical_semantic_memory_20260717}
...
run_scene() {
  ...
  local memory=$scene_root/hierarchical_memory
  ...
  if [[ ! -f "$memory/manifest.json" ]]; then
    "$PYTHON_BIN" -u build_hierarchical_semantic_memory.py \
      --geometry_checkpoint "$ROOT/runs/3dgs/$scene/chkpnt30000.pth" \
      --old_consensus "$old" \
      ...
      --output_dir "$memory" --device cuda --neighbors 8 \
      --semantic_thresholds 0.76 0.82 0.87 0.91 \
      --maximum_group_sizes 2048 512 128 32 \
      --minimum_group_sizes 16 8 4 2 \
      --stability_floor 0.5 --minimum_reliability 0.25 --source_margin 0.0 \
  ...
  if [[ ! -f "$output/metrics.json" ]]; then
    ...
    "$PYTHON_BIN" -u eval_lerf_ovs_gaussian_codebook_miou.py \
      ...
      --group_topk 4 --group_readout hierarchical_memory \
  ...
  "$PYTHON_BIN" -u eval_gaussian_split_query_consistency.py \
    ...
    --a26_group_dir "$memory" --samples 100000 \
...
for index in "${!scenes[@]}"; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py --gpu "$gpu" --hold-mb 384 \
    ...
    bash "$ROOT/scripts/run_a26_hierarchical_semantic_memory.sh" --worker "$scene" \
...
for scene in "${scenes[@]}"; do
  "$PYTHON_BIN" analyze_small_object_metrics.py \
    ...
    --metrics \
      ...
      "A26@${SELECTION_THRESHOLD}=$RUN_ROOT/$scene/eval/metrics.json" \
...
for scene in scenes:
    ...
    summary["scenes"][scene] = {
        ...
        "a26": row(os.path.join(root, scene, "eval", "metrics.json"), threshold),
        "consistency": consistency,
        "small_object": small,
        "hierarchy": memory["hierarchy"],
        "vocabulary": memory["modality_token_counts"],
    }
for method in ("drsplat_pq_baseline", "a14", "a20", "a24", "a26"):
    summary[method + "_mean"] = {
        metric: sum(summary["scenes"][scene][method][metric] for scene in scenes) / len(scenes)
        for metric in metrics
    }
summary["a26_minus_a20_mean"] = {
    metric: summary["a26_mean"][metric] - summary["a20_mean"][metric]
    for metric in metrics
}
for method, key in {
...
summary["decision"] = {
    "beats_a20_mean_miou": summary["a26_minus_a20_mean"]["mIoU"] > 0.0,
    "small_object_at_least_a24": summary["small_object_mean"]["a26"] >= summary["small_object_mean"]["a24"],
    "ramen_macc_at_least_a20": (
        "ramen" not in scenes
        or summary["scenes"]["ramen"]["a26"]["mAcc@0.5"] >= summary["scenes"]["ramen"]["a20"]["mAcc@0.5"]
    ),
    "consistency_not_worse_than_a20": (
        summary["a26_consistency_mean"]["canonical_split_symmetric_kl"] <= summary["a20_consistency_mean"]["canonical_split_symmetric_kl"]
        and summary["a26_consistency_mean"]["canonical_split_top1_flip_rate"] <= summary["a20_consistency_mean"]["canonical_split_top1_flip_rate"]
    ),
    "all_four_sam_levels_present": all(
        all(summary["scenes"][scene]["vocabulary"].get(level, 0) > 0 for level in ("sam_l0", "sam_l1", "sam_l2", "sam_l3"))
        for scene in scenes
    ),
}
