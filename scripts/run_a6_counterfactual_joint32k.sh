...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/a6_counterfactual_joint32k_20260715}
LOG_DIR=${LOG_DIR:-$ROOT/logs/a6_counterfactual_joint32k_20260715}
...
split_path() {
  [[ "$1" == "waldo_kitchen" ]] && printf '%s\n' "$ROOT/runs/multiscale_split_consistency/waldo_l2_split2/consensus.pt" || printf '%s\n' "$ROOT/runs/multiscale_split_consistency/$1/l2_split2/consensus.pt"
}

build_mask() {
  local scene=$1
  mkdir -p "$RUN_ROOT/$scene"
  "$PYTHON_BIN" build_counterfactual_route_mask.py \
    --base_consensus "$(base_path "$scene")" \
    --candidate_consensus "$(candidate_path "$scene")" \
    --split_consensus "$(split_path "$scene")" \
    --min_advantage 0 \
...
run_scene() {
  ...
  "$PYTHON_BIN" -u eval_lerf_ovs_gaussian_codebook_miou.py \
    ...
    --query_route_candidate_mask "$RUN_ROOT/$scene/candidate_mask.npy" \
...
for gpu in 0 1 2 3; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py \
    ...
    --wait-timeout 0 -- \
    bash "$ROOT/scripts/run_a6_counterfactual_joint32k.sh" --worker "$scene" \
...
echo "A6 counterfactual joint-32k evaluation complete: $RUN_ROOT"
