...
RUN_ROOT=${RUN_ROOT:-$ROOT/runs/e12_spatial_support_blend_20260716}
LOG_DIR=${LOG_DIR:-$ROOT/logs/e12_spatial_support_blend_20260716}
...
for index in "${!scenes[@]}"; do
  ...
  "$PYTHON_BIN" scripts/gpu_guard.py \
    ...
    --wait-timeout 0 -- \
    bash "$ROOT/scripts/run_e12_spatial_support_blend.sh" --worker "$scene" \
...
for scene in scenes:
    rows = {}
    for name, root in (("e8_3", baseline_root), ("e12", run_root)):
    ...
    rows["delta"] = {key: rows["e12"][key] - rows["e8_3"][key] for key in rows["e12"]}
    summary["scenes"][scene] = rows
for name in ("e8_3", "e12", "delta"):
    summary[name + "_mean"] = {
...
echo "E12 spatial support blend probe complete: $RUN_ROOT"
